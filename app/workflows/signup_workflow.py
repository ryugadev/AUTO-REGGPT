...
from app.workflows.browser_workflow import AccountExistsError, BrowserPhaseError, run_browser_phase
from app.core.config import load_settings, runtime_session_dir
from app.workflows.http_workflow import HttpPhaseError, run_http_phase
from app.providers.otp.gmail_otp_provider import AutoGmailOtpError
from app.providers.mail.base import MailProvider
from app.providers.mail.outlook_provider import (
    OutlookComboError,
    OutlookProviderUnavailable,
    build_provider_outlook,
)
from app.providers.mail.gmail_provider import (
    build_provider_gmail_advanced,
    build_provider_auto_gmail_otp,
)
from app.providers.mail.worker_provider import build_provider_worker
from app.models import SignupRequest, SignupResult
from app.utils.helpers import random_profile
...
def _build_mail_provider(request: SignupRequest, *, settings) -> MailProvider:
    ...
    if request.mail_provider == "auto_gmail_otp":
        # api_key + service đọc từ Settings (.env) — KHÔNG hardcode.
        if not settings.shopgmail_api_key:
            raise ValueError("Missing SHOPGMAIL_API_KEY")
        if not settings.shopgmail_otp_service:
            raise ValueError("Missing SHOPGMAIL_OTP_SERVICE")
        return build_provider_auto_gmail_otp(
            api_key=settings.shopgmail_api_key,
            service=settings.shopgmail_otp_service,
            proxy=request.proxy,
        )
    ...
    try:
        ...
        if hasattr(provider, "pre_check"):
            try:
                ...
                # Gắn metadata provider nếu có (Auto Gmail OTP: email/order_id/service)
                if hasattr(provider, "get_mail_meta"):
                    result.mail_meta = provider.get_mail_meta()
            # Guard: nếu sau pre_check vẫn không có email thật → fail
            if not provider.email or provider.email == "pending@gmail-advanced.local":
                provider_name = getattr(provider, "mode_name", "Mail Provider")
                raise ValueError(
                    f"{provider_name}: API không trả email, không thể tiếp tục signup"
                )
        ...
        # Account đã đăng ký từ trước — không phải lỗi combo. Dùng marker rõ ràng
        # để manager hiển thị trạng thái 'registered' thay vì error đỏ.
        result.error = "account_already_registered"
        log(f"[signup] account already registered: {exc}")
    except (
        BrowserPhaseError,
        HttpPhaseError,
        TimeoutError,
