...
from app.core.config import Settings, ensure_runtime_dirs, prepare_profile_dir
from app.providers.mail.base import MailProvider
from app.models import BrowserHandoff, SignupRequest
from app.infrastructure.browser.nextauth import bootstrap_authorize_url
from app.infrastructure.browser.retry import (
...
class AccountExistsError(BrowserPhaseError):
    """Account đã đăng ký ChatGPT từ trước (gặp màn hình login password).

    Kế thừa BrowserPhaseError để mọi handler bắt BrowserPhaseError vẫn hoạt động,
    nhưng phân biệt được để UI hiển thị trạng thái riêng (không phải lỗi combo)."""
...
async def _submit_otp(page, *, otp_code: str, otp_selector: str, log) -> None:
    """Fill OTP + click submit + gọi validate API trực tiếp (dual submit).

    Hotmail flow hay bị stuck sau button click vì SPA không redirect.
    Dual submit: click button UI trước, rồi gọi API validate luôn — nếu 1 trong 2
    thành công, page sẽ chuyển.
    """
    ...
    for btn in ('button[type="submit"]', 'button:has-text("Continue")', 'button:has-text("Verify")'):
        try:
            ...
            continue
    # Luôn gọi API validate trực tiếp sau button click (hoặc nếu không có button).
    # API call idempotent — gọi 2 lần không hại gì, nhưng tăng reliability.
    await asyncio.sleep(0.5)
    try:
        ...
        api_status = validate_result.get("status") if isinstance(validate_result, dict) else None
        log(f"[browser] API validate result: HTTP {api_status}")
    except Exception as exc:
        log(f"[browser] API validate call failed (non-fatal): {exc}")
    if not button_clicked:
        log("[browser] no submit button found — relied on API validate")
    ...
    *, ctx, page, request, mail_provider, callback_holder, otp_started_at, log,
    ...
    while time.monotonic() < deadline:
        ...
        if screen == "password_login":
            raise AccountExistsError("account_already_registered")
        ...
        if screen == "otp":
            ...
            if otp_submitted:
                # Đã submit rồi, đợi page chuyển. Nếu stuck quá lâu → retry.
                if same_screen_count > 15:  # ~7.5s ở cùng OTP screen sau submit
                    log("[flow] OTP screen vẫn ở đây sau submit — retry validate API + click")
                    # Gọi API validate trực tiếp (lần nữa) — cover case button click bị nuốt
                    try:
                        otp_inp = page.locator('input[name="code"]').first
                        current_code = await otp_inp.input_value()
                        if current_code and len(current_code) == 6:
                            await page.evaluate(
                                """async (code) => {
                            ...
                            log(f"[flow] re-called API validate with code={current_code}")
                    except Exception:
                        pass
                    for btn in ('button[type="submit"]', 'button:has-text("Continue")', 'button:has-text("Verify")'):
                        try:
                            ...
                            continue
                    same_screen_count = 0
                # Stuck quá 30 lần (~15s) → giả định OTP code sai dù không có error visible → re-poll
                if same_screen_count > 30:
                    log("[flow] OTP stuck >15s không có error visible — re-poll code mới")
                    otp_submitted = False
                    same_screen_count = 0
                    try:
                        otp_inp = page.locator('input[name="code"]').first
            ...
            # Lùi 5 giây để bù clock skew giữa local server và mail provider
            # (Microsoft Graph receivedDateTime có thể lệch vài giây so với UTC local).
            from datetime import timedelta
            poll_started = (datetime.now(timezone.utc) - timedelta(seconds=5)).replace(microsecond=0)
...
async def _wait_after_otp(page, *, timeout_seconds: float, log) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        # Hoặc nếu 8s trôi qua và vẫn stuck ở /email-verification → thử validate + click lại
        elapsed = time.monotonic() - (deadline - timeout_seconds)
        if elapsed > 8.0 and not otp_resubmit_attempted:
            try:
                otp_input = page.locator('input[name="code"]').first
                if await otp_input.is_visible(timeout=500):
                    val = await otp_input.input_value()
                    if val and len(val) == 6:
                        log("[browser] OTP form still visible after 8s — retry validate API + click")
                        # Gọi API validate trực tiếp
                        try:
                            await page.evaluate(
                                """async (code) => {
                                    await fetch('/api/accounts/email-otp/validate', {
                            ...
                            log(f"[browser] re-called API validate with code={val}")
                        except Exception:
                            pass
                        for btn in ('button[type="submit"]', 'button:has-text("Continue")'):
                            try:
    ...
    try:
        ...
        if date_input:
            ...
            if age_input:
                ...
                # QUAN TRỌNG: form /about-you mới có hidden field `birthday` được React
                # tính từ age CHỈ KHI input age bị blur. Không blur → birthday giữ giá trị
                # mặc định (hôm nay = 0 tuổi) → submit bị validation chặn, đứng yên.
                await page.keyboard.press("Tab")
                log(f"[browser] typed age={age} + blur")
            else:
                await page.keyboard.press("Tab")
                await asyncio.sleep(0.4)
                await page.keyboard.type(str(age), delay=120)
                await page.keyboard.press("Tab")
                log(f"[browser] Tab + typed age={age} + blur")

        await asyncio.sleep(0.5)

        # Verify hidden `birthday` đã commit (khác hôm nay). Nếu chưa → set trực tiếp
        # qua native setter + dispatch input/change để React nhận giá trị.
        try:
            from datetime import date as _date
            today_iso = _date.today().isoformat()
            bday_val = await page.evaluate(
                "() => { const e = document.querySelector('input[name=\"birthday\"]'); return e ? e.value : null; }"
            )
            if bday_val is not None and (not bday_val or bday_val == today_iso):
                target_iso = f"{datetime.utcnow().year - age}-{datetime.utcnow():%m-%d}"
                log(f"[browser] birthday chưa commit ({bday_val}) — set trực tiếp {target_iso}")
                await page.evaluate(
                    """(iso) => {
                        const el = document.querySelector('input[name="birthday"]');
                        if (!el) return;
                        const desc = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(el), 'value');
                        desc.set.call(el, iso);
            ...
            log(f"[browser] birthday verify skip: {exc}")

        # Submit. Form mới dùng nút "Finish creating account".
        for btn in (
        ...
        while time.monotonic() < deadline:
            ...
            if not submitted_again and time.monotonic() > retry_submit_at and "about-you" in cur:
                ...
                log("[browser] still on /about-you after 10s — re-commit birthday + retry submit")
                # Re-blur age + ép set hidden birthday (phòng React chưa commit)
                try:
                    age_el = page.locator('input[name="age"]').first
                    if await age_el.is_visible(timeout=300):
                        await age_el.click(force=True, timeout=1500)
                        await page.keyboard.press("Tab")
                        await asyncio.sleep(0.3)
                    from datetime import date as _date2
                    today_iso2 = _date2.today().isoformat()
                    bday_now = await page.evaluate(
                        "() => { const e = document.querySelector('input[name=\"birthday\"]'); return e ? e.value : null; }"
                    )
                    if bday_now is not None and (not bday_now or bday_now == today_iso2):
                        await page.evaluate(
                            """() => {
                                const a = document.querySelector('input[name="age"]');
                                const b = document.querySelector('input[name="birthday"]');
                                if (!a || !b) return;
                                const age = parseInt(a.value || '0', 10);
                                if (!age) return;
                                const d = new Date();
                                const iso = (d.getFullYear() - age) + '-' +
                                    String(d.getMonth()+1).padStart(2,'0') + '-' +
                                    String(d.getDate()).padStart(2,'0');
                                const desc = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(b), 'value');
                                desc.set.call(b, iso);
